...
namespace DragDrapWatcher_AddIn
{
  public class SuperMailFolder
  {
    ...
    private void Before_ItemMoveListener(object Item, MAPIFolder TargetFolder, ref bool Cancel)
    {
      ...
      try
      {
        Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Drag and drop triggered!");
        if (Item is MailItem && TargetFolder != null)
        {
          Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Drag and drop happening for folder : {TargetFolder.Name}");
          if (!TargetFolder.Name.StartsWith("deleted items", StringComparison.OrdinalIgnoreCase))
          {
            ...
            {
              Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Sender Address is Null or empty!");
              return;
            }
            ...
            if (src_folder.Name.StartsWith(folder_prefix, StringComparison.OrdinalIgnoreCase))
            {
              ...
              {
                Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Removing {row.sender_email} from {row.rulename} !");
            ...
            {
              Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Skip scanning source folder as it didn't start with #");
            }
            ...
            if (TargetFolder.Name.StartsWith(folder_prefix, StringComparison.OrdinalIgnoreCase))
            {
              ...
              Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Adding {sender_address} to {target_ruleprefix} on {TargetFolder.Name}!");
              ok_added = Globals.ThisAddIn.OutlookRules.AddEmailToRule(target_ruleprefix, sender_address, oMsg.SenderName, TargetFolder);
            }
            else
            {
              Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Skip scanning target folder as it didn't start with #");
            }

            Globals.ThisAddIn.Error_Sender.WriteLog($"{this.GetType().Name}->{MethodBase.GetCurrentMethod().Name} :: Drag and drop for {sender_address}, ok_added : {ok_added}, ok_removed : {ok_removed}");
